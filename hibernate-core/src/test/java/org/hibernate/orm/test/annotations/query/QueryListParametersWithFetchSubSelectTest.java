...
public class QueryListParametersWithFetchSubSelectTest {
	...
	public void simpleTest(SessionFactoryScope scope) {
		...
		scope.inTransaction( s -> {
			TypedQuery<Parent> query = s.createQuery( "select p from Parent p where id in :ids order by id",
					Parent.class );
	...
	public void reusingQueryWithFewerOrdinalParameters(SessionFactoryScope scope) {
		...
		scope.inTransaction( s -> {
			TypedQuery<Parent> query = s.createQuery( "select p from Parent p where id in ?1 order by id",
					Parent.class );
